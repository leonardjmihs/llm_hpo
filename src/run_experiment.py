...
def llama3_hpo():
    ...
    optimization_method_name=OPTIMIZATION_METHODS.optuna.name
    # llm_name = "deepseek-ai/DeepSeek-R1-Distill-Qwen-7B"
    llm_name="deepseek-r1:14b"
    ml_model_name=ML_MODEL_NAMES.lightgbm.name
    dataset_name = DATASET_NAMES.gas_drift.name
...
def optuna_hpo():
    ...
    llm_tpe_init_method_name = LLM_TPE_INIT_METHODS.random.name
    # sllmbo_method_name = SLLMBO_METHODS.sllmbo_llm_tpe.name
    patience = None
    optimization_args = {
            "X": X_train,
    ...
    experiment_name = f"optuna_{dataset_name}_{ml_model_name}_{llm_tpe_init_method_name}_init"  # noqa: E501
    if patience is not None and patience > 0:
        experiment_name += f"_patience_{patience}"
    print(experiment_name)
    storage = (
